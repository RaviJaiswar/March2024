...
Comb = fread('ZFOURGE-ProSpect-new.csv',sep=',')
...
shh = foreach(ii = 1:nrow(Comb))%do%{
  ...
  Data=list(flux= getinfo(GalID)$flux,
            arglist=list(z=redshift, 
                         emission =FALSE,
                         massfunc=massfunc_exp, ### use this to change SFH parameterisations -- use ?massfunc to see some of the inbuilt options 
                         agemax=agemax,
                        Zagemax = (agemax/1e9), #Choose a fixed value to compare to MAGPHYS
                        veldisp = 100,
                        magemax=(agemax/1e9),
                        Z=Zfunc_massmap_lin, ### THIS CHANGES THE METALLICITY HISTORY -change to a fixed value 0.02
            ...
            parm.names=c('mSFR', 'mtau', 'mpivot','tau_birth','tau_screen', 'alpha_SF_birth','alpha_SF_screen', 'Zfinal'),
            ...
            logged=c(T,F,F,T,T,F,F,T), # fit parameters in logged or linear space
            intervals=list(lo=c(-3,0,0,-2.5,-5,0,0,-4),
            hi=c(4,3,agemax/1e9,1.5,1,4,4,-1.3)), # fitting range for parameters
  ...
  #pdf(output,file = paste0(GalID, '.pdf' ), width = 11, height = 8)
  #plot(bestfit)
  dev.off()
  ...
  write.table(SEDstarsunattendata, file=paste0(OutputDir,GalID,'_SED_unatten.txt', sep=''), row.names=FALSE, col.names=TRUE)   
  #print(OutputDir)
  write.table(SEDdata, file=paste0(OutputDir,GalID,'_SED_bestfit.txt', sep=''), row.names=FALSE,   col.names=TRUE)
  print('test1')
  write.table(SEDbanddata, file=paste0(OutputDir,GalID,'_SED_bands_data.txt', sep=''), row.names=FALSE, col.names=TRUE)
  print('test2')
  write.table(SEDbandmodel, file=paste0(OutputDir,GalID,'_SED_bands_bestfit.txt', sep=''), row.names=FALSE, col.names=TRUE)
  print('done!')
